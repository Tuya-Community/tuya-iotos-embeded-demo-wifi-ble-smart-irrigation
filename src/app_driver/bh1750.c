...
STATIC VOID __bh1750_send_ack(i2c_ack ack)
{
    if(TRUE == ack) {
        vI2CSDASet();
    }else {
...
{   
    ...
    temp = (float)data/1.2;
    PR_NOTICE("light sensor value = %f",temp);
...
UCHAR_T tuya_bh1750_init(bh1750_init_t* bh1750_init_param)
{   
    UCHAR_T error = 0;
    ...
    i2c_pin_t i2c_config = {
        .ucSDA_IO = bh1750_init_param ->SDA_PIN,
        .ucSCL_IO = bh1750_init_param ->SCL_PIN,
    };
